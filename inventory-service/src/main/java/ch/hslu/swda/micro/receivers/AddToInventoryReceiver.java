 ...
 * Returns: OrderInfo[]
 */
public final class AddToInventoryReceiver implements MessageReceiver {

    private static final Logger LOG = LoggerFactory.getLogger(AddToInventoryReceiver.class);
    private final String exchangeName;
    private final BusConnector bus;
    private final ObjectMapper mapper;
    private final Inventory inventory;

    public AddToInventoryReceiver(final String exchangeName, final BusConnector bus, Inventory inventory) {
    ...
    public void onMessageReceived(final String route, final String replyTo, final String corrId, final String message) {
        ...
        try {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                orderInfo[i] = this.inventory.add(item.getProductId(), item.getQuantity());
            }
